...
class SocketChat
{
    ...
    private static $connectPool = array();  //连接池
    private static $maxConnectNum = 1024; //最大连接数
    private static $chatUser = array();  //参与聊天的用户
    ...
    public function startServer()
    {
        ...
        $this->runLog("Server Started : ".date('Y-m-d H:i:s', time()));
        $this->runLog("Listening on   : 192.168.3.132 port " . $this->port);
                ...
                if( $this->master == $socket ){
                    ...
                    if( $bytes == 0 ){
                        ...
                        if( !$this->handShake ){
                            ...
                            $this->printData($buffer, 'buffer');
                            $buffer = $this->decode( $buffer );
                            $this->printData($buffer, 'message');
    ...
    public function parseMessage( $message, $socket )
    {
        ...
        $this->printData($message, 'message_array');
        switch( $message['type'] ){
    ...
    {
        self::$chatUser[(int) $socket] = array(
            'user' => $user['user'],
            'avar' => $user['avar']
        );
    }
    ...
    public function sendToAll( $client, $mess )
    {
        ...
        $mess['stime'] = date('Y-m-d H:i:s', time());
   ...
   public function send( $client, $msg )
    ...
    function connect( $socket )
    {
        ...
        $this->runLog("\n" . $socket . " CONNECTED!");
        $this->runLog(date("Y-n-d H:i:s", time()));
    }
    ...
    {
        @file_put_contents( './' . date("Y-m-d", time()) . ".log", date('Y-m-d H:i:s', time()) . "  " . $mess . PHP_EOL, FILE_APPEND );
    }

    public function printData($data, $title)
    {
        echo $title . ':' . PHP_EOL;
        print_r($data);
        echo PHP_EOL;
